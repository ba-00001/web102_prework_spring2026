...
const totalContributions = games.reduce((acc, game) => acc + game.backers, 0);
...
const totalRaised = games.reduce((acc, game) => acc + game.pledged, 0); //*
...
const unfundedGamesCount = games.filter(game => game.pledged < game.goal).length;

//* create a string that explains the number of unfunded games using the ternary operator
const unfundedGamesMessage = `We currently have ${unfundedGamesCount} unfunded games ${(unfundedGamesCount > 0) ? "awaiting support." : "ready to launch!"}`;

//* create a new DOM element containing the template string and append it to the description container
const unfundedGamesInfo = document.createElement("p");
unfundedGamesInfo.textContent = unfundedGamesMessage;
